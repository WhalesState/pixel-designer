...
func _init(_editor):
	...
	vp_width.value_changed.connect(func(value: float):
		editor.node_editor.root_container.size.x = value
		editor.node_editor.view_size.x = value
	...
	vp_height.value_changed.connect(func(value: float):
		editor.node_editor.root_container.size.y = value
		editor.node_editor.view_size.y = value
	...
	snap_transforms.toggled.connect(func(enabled: bool):
		editor.node_editor.root.snap_2d_transforms_to_pixel = enabled
	...
	snap_vetrices.toggled.connect(func(enabled: bool):
		editor.node_editor.root.snap_2d_vertices_to_pixel = enabled
...
func update_settings():
	if not editor.node_editor:
		return
	var vp_size := editor.get_viewport_size()
	vp_width.value = vp_size.x
	vp_height.value = vp_size.y
	snap_transforms.button_pressed = editor.node_editor.root.snap_2d_transforms_to_pixel
	snap_vetrices.button_pressed = editor.node_editor.root.snap_2d_vertices_to_pixel
