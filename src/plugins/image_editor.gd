...
var add_texture_window := AddTextureWindow.new(self)
...
func _init(_editor: Editor):
	...
	root.canvas_item_default_texture_filter = project_settings.get_value("project", "filter", Viewport.DEFAULT_CANVAS_ITEM_TEXTURE_FILTER_NEAREST)
	...
	design_menu.connect("id_pressed", func(id: int):
		if id == 0:
			add_texture_window.popup_centered()
		elif id == 1:
			pass
		elif id == 2:
			pass
	...
	if edit_mode:
		...
		if mb:
			if mb.button_index == MOUSE_BUTTON_LEFT:
				...
				var w: Window = get_tree().get_root()
				var mpos = w.get_screen_transform()._xform(w.get_mouse_position() + Vector2(w.position))
				design_menu.popup(Rect2i(mpos.x, mpos.y, 0, 0))
